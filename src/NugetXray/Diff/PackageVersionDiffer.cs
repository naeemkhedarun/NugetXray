    ...
    public static class PackageVersionDiffer
    {
        public static PackageDiff GetVersionDiff(PackageReference packageReference, NuGetVersion[] versions)
        {
            ...
            if (latest.Major != current.Major)
                return new PackageDiff(packageReference, current, latest, new SemanticVersion(latest.Major - current.Major, 0, 0));
            if (latest.Minor != current.Minor)
                return new PackageDiff(packageReference, current, latest, new SemanticVersion(0, latest.Minor - current.Minor, 0));
            return new PackageDiff(packageReference, current, latest, new SemanticVersion(0, 0, latest.Patch - current.Patch));
        }
    }
}